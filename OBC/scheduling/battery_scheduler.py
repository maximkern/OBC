...
state_processes = [ # insert relative paths to these files
    ...
    root_dir + "/scheduling/state_process/state_antennas.py",           # needs a file
    root_dir + "/scheduling/state_process/state_comms.py",              # needs a file
    root_dir + "/scheduling/state_process/state_payload.py"             # needs a file
]
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            startracker_data = random.uniform(0,10)
            angular_velocity = random.uniform(0,.011)
            if battery >= 95:
                # deploy antennas
                print("antennas deployed")
                stop_event103 = multiprocessing.Event()
                process103 = multiprocessing.Process(target=run_script,args=(state_processes[], output_queue, stop_event103, 103)) )
